...
RUN_METADATA_ENV = {
    "run_id": "ASCENT_RUN_ID",
    "git_commit": "ASCENT_GIT_COMMIT",
    "git_branch": "ASCENT_GIT_BRANCH",
    "baseline_commit": "ASCENT_BASELINE_COMMIT",
    "command": "ASCENT_RUN_COMMAND",
    "config": "ASCENT_RUN_CONFIG",
    "seed": "ASCENT_RUN_SEED",
    "workspace_snapshot": "ASCENT_WORKSPACE_SNAPSHOT",
}


def run_metadata_from_env() -> Dict[str, str]:
    metadata = {"schema": "instrumentation-only-v1"}
    for key, env_name in RUN_METADATA_ENV.items():
...
class DecisionTraceWriter:
    ...
    def from_env(cls, num_envs: int) -> "DecisionTraceWriter":
        ...
        if not enabled:
            return cls(enabled=False, output_dir=Path("."))
    ...
    def _extract_topdown_trace(
        ...
        return {
            ...
            "stop_action": _is_stop_action(action),
        }
