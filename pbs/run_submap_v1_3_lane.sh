#!/bin/bash
# One B2-only ASCENT-VO submap-v1.3 lane with independent services.
...
DATASET=${ASCENT_SUBMAP_V13_DATASET:?required}
MODE=${ASCENT_SUBMAP_V13_MODE:?required}
SCENES_ROOT=${ASCENT_SUBMAP_V13_SCENES_ROOT:?required}
MANIFEST=${ASCENT_SUBMAP_V13_MANIFEST:?required}
MANIFEST_SHA256=${ASCENT_SUBMAP_V13_MANIFEST_SHA256:?required}
SOURCE_COMMIT=${ASCENT_SUBMAP_V13_SOURCE_COMMIT:?required}
RESOURCE_COMMIT=${ASCENT_SUBMAP_V13_RESOURCE_COMMIT:?required}
POINTNAV_VO_COMMIT=${ASCENT_SUBMAP_V13_POINTNAV_VO_COMMIT:?required}
LANE_ID=${ASCENT_SUBMAP_V13_LANE_ID:?required}
LANE_COUNT=${ASCENT_SUBMAP_V13_LANE_COUNT:?required}
EXPECTED_CHUNKS=${ASCENT_SUBMAP_V13_LANE_EXPECTED_CHUNKS:?required}
EXPECTED_EPISODES=${ASCENT_SUBMAP_V13_LANE_EXPECTED_EPISODES:?required}
STAGE_ROOT=${ASCENT_SUBMAP_V13_STAGE_ROOT:?required}
BASE_PORT=${ASCENT_SUBMAP_V13_BASE_PORT:?required}
DEADLINE_EPOCH=${ASCENT_SUBMAP_V13_DEADLINE_EPOCH:?required}
...
{
  echo schema=ascent_vo_submap_v1_3_lane_v1
  ...
  echo automatic_technical_retry_per_failed_unit=0
  echo method_version=submap_v1.3
  echo handoff_enabled=1
  echo exhaustion_recovery_enabled=1
  echo target_relay_enabled=1
...
run_unit() {
  ...
  local cmd=(
    ...
    "ascent_submaps.target_relay_enabled=true"
  ...
  )
  {
    ...
    printf 'ASCENT_SUBMAP_TARGET_RELAY_ENABLED=true '
  ...
  if (
    ...
    export ASCENT_SUBMAP_TARGET_RELAY_ENABLED=true
...
if [ "${#FAILED_UNITS[@]}" -gt 0 ]; then
  echo "technical_units_missing_without_automatic_retry=${#FAILED_UNITS[@]}"
  exit 44
fi
