#!/bin/bash
# Submit one hash-bound B2-only ASCENT-VO submap-v1.1 screen.

set -euo pipefail

USAGE='submit_submap_v1_1.sh hm3d|mp3d smoke|full [auto|autox] [stamp]'
DATASET=${1:?usage: $USAGE}
MODE=${2:?usage: $USAGE}
QUEUE=${3:-autox}
STAMP=${4:-20260730_submap_v1_1_${DATASET}_${MODE}_first_attempt}
NO_QSUB=${ASCENT_SUBMAP_V11_NO_QSUB:-0}

PROJECT=/scratch/e1538633/liuyi/drift-aware-submap-exploration
SOURCE_ROOT=$PROJECT/external/ascent_vo_submap_v1_1
RESOURCE_ROOT=$PROJECT/external/ascent
POINTNAV_VO_ROOT=$PROJECT/external/PointNav-VO
ARTIFACT_ROOT=${ASCENT_SUBMAP_V11_ARTIFACT_ROOT:-$PROJECT/artifacts/objectnav/submap_v1_1}
MANIFEST_ROOT=$PROJECT/artifacts/objectnav/submap_v1/manifests
CONTROLLER=$SOURCE_ROOT/pbs/run_submap_v1_1_3shared.pbs
WORKER=$SOURCE_ROOT/pbs/run_submap_v1_1_lane.sh
...
print(json.dumps({
    "status": "PASS",
    "schema": "ascent_vo_submap_v1_1_submission_preflight_v1",
    "dataset": "$DATASET",
    "mode": "$MODE",
    "queue_request": "$QUEUE",
    "walltime": "$WALLTIME",
...
PBS_LOG=$LOG_ROOT/submap_v1_1_${DATASET}_${MODE}_${STAMP}.pbs.log
SUBMISSION_JSON=$SUBMISSION_ROOT/submap_v1_1_${DATASET}_${MODE}_${STAMP}.json
...
"ASCENT_SUBMAP_V11_DATASET=$DATASET,\
ASCENT_SUBMAP_V11_MODE=$MODE,\
ASCENT_SUBMAP_V11_ARTIFACT_ROOT=$ARTIFACT_ROOT,\
ASCENT_SUBMAP_V11_MANIFEST=$MANIFEST,\
ASCENT_SUBMAP_V11_MANIFEST_SHA256=$MANIFEST_SHA256,\
ASCENT_SUBMAP_V11_EXPECTED_CHUNKS=$EXPECTED_CHUNKS,\
ASCENT_SUBMAP_V11_EXPECTED_EPISODES=$EXPECTED_EPISODES,\
ASCENT_SUBMAP_V11_GATE_MANIFEST=$SMOKE_MANIFEST,\
ASCENT_SUBMAP_V11_GATE_MANIFEST_SHA256=$SMOKE_MANIFEST_SHA256,\
ASCENT_SUBMAP_V11_SOURCE_COMMIT=$SOURCE_COMMIT,\
ASCENT_SUBMAP_V11_RESOURCE_COMMIT=$RESOURCE_COMMIT,\
ASCENT_SUBMAP_V11_POINTNAV_VO_COMMIT=$POINTNAV_VO_COMMIT,\
ASCENT_SUBMAP_V11_CONTROLLER_SHA256=$CONTROLLER_SHA256,\
ASCENT_SUBMAP_V11_WORKER_SHA256=$WORKER_SHA256,\
ASCENT_SUBMAP_V11_STAMP=$STAMP,\
ASCENT_SUBMAP_V11_BASELINE_CSVS=$BASELINE_CSVS")
...
output.update({
    "schema": "ascent_vo_submap_v1_1_submission_v1",
    "job_id": sys.argv[2],
    "pbs_log": sys.argv[3],
    "qsub_executed": True,
})
