#!/usr/bin/env python3
"""Strict B2-only submap-v1.1 gate against frozen pose-factorial arm A."""
...
def main() -> int:
    ...
    mechanism_exposure = (
        "ROUTE_EXPOSED"
        if route_selected > 0
        else ("SPLIT_ONLY" if split_count > 0 else "NO_EXPOSURE")
    )
    ...
    summary = {
        "schema": "ascent_vo_submap_v1_1_b2_screen_v1",
        "dataset": args.expected_dataset,
        "mode": args.mode,
