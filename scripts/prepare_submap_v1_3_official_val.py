#!/usr/bin/env python3
"""Prepare hash-bound official-val shards for frozen submap-v1.3.
    ...
    *,
    ...
    for local_index, row in enumerate(selected):
        ...
        selection_rows.append(
            {
                "manifest_version": (
                    "ascent_vo_submap_v1_3_official_val_expansion_v1"
                ),
                "selection_rule": (
...
def prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    smoke_selection = output_root / "smoke_selection.csv"
    write_csv(smoke_selection, selection_rows[:5], SELECTION_FIELDS)
    smoke_gate = materialize_view(
        selection=smoke_selection,
        output_root=output_root / "smoke_materialized",
        scene_dataset_config=scene_dataset_config,
        dataset=args.dataset,
        episodes=5,
        chunk_size=5,
        label=f"{args.label}_smoke5",
    )
    ...
    for shard_index, size in enumerate(sizes):
        ...
        audit = {
            "schema": (
                "ascent_vo_submap_v1_3_official_val_preparation_v1"
            ),
            "status": "PASS",
            "dataset": args.dataset,
            "split": "val",
            ...
            "gate_manifest": smoke_gate["manifest_path"],
            "gate_manifest_sha256": smoke_gate["manifest_sha256"],
        }
        write_json(audit_path, audit)
        config_path = shard_root / "job_config.json"
        job_key = f"{args.label}_s{shard_index}"
        config = {
            "schema": "ascent_vo_submap_v1_3_official_val_job_v1",
            "job_key": job_key,
            "job_name": (
                f"v13h{shard_index}"
                if args.dataset == "hm3d"
                else f"v13m{shard_index}"
            ),
            "dataset": args.dataset,
            ...
            "gate_manifest": smoke_gate["manifest_path"],
            "gate_manifest_sha256": smoke_gate["manifest_sha256"],
            "smoke_logical_case_ids": [
                row["logical_case_id"] for row in selection_rows[:5]
            ],
            "selection": str(selection_path),
            "selection_sha256": sha256(selection_path),
    ...
    plan = {
        "schema": "ascent_vo_submap_v1_3_official_val_plan_v1",
        "status": "PASS",
        "dataset": args.dataset,
        "expected_official_total": args.expected_total,
        ...
        "smoke_selection": str(smoke_selection),
        "smoke_selection_sha256": sha256(smoke_selection),
        "smoke_manifest": smoke_gate["manifest_path"],
        "smoke_manifest_sha256": smoke_gate["manifest_sha256"],
